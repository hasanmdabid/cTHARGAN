...
def define_discriminator(in_shape=(32, 107, 1), n_classes=18):
    ...
    dis = Conv2D(filters=64, kernel_size=(3, 1), strides=(1, 1), padding='same', name='Discriminator-Hidden-Layer-1')(merge)  # 16x54x64
    dis = BatchNormalization(momentum=0.5)(dis)
    dis = LeakyReLU(alpha=0.2, name='Discriminator-Hidden-Layer-Activation-1')(dis)
    dis = Dropout(0.2)(dis)

    # down-sample
    dis = Conv2D(filters=128, kernel_size=(3, 1), strides=(1, 1), padding='same', name='Discriminator-Hidden-Layer-2')(dis)  # 8x27x128
    dis = BatchNormalization(momentum=0.5)(dis)
    dis = LeakyReLU(alpha=0.2, name='Discriminator-Hidden-Layer-Activation-2')(dis)
    dis = Dropout(0.2)(dis)

    #dis = MaxPool2D(pool_size=(2, 1), strides=(1, 1), padding='valid', name='Discriminator-MaxPool-Layer-2')(dis)  # Max Pool 3*13*128

    # flatten feature maps
    dis = Flatten(name='Discriminator-Flatten-Layer')(dis)  # 4992  (3*13*128=8192)
    # Dense Layer
    ...
    # output
    out_layer = Dense(1, activation='sigmoid', name='Discriminator-Output-Layer')(dis)  # Shape=1
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch):
    ...
    g_model.save('CGAN_OPPO_100_epochs_64Batch_With_BatchNormalization.h5')
