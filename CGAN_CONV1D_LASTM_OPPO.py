...
#--------- Checking whether all the Feature value is scaled between +1 and -1 ------------------------------------------
max_val = np.max(trainx_2D_scaled)
min_val = np.min(trainx_2D_scaled)
print('Maximume_value after scaled:', max_val)
print('Minimume_value after scaled:', min_val)

X = trainx_2D_scaled.reshape(nr_samples, nr_rows, nr_columns)
print("Shape of the scaled array: ", X.shape)
dataset = [X, trainy]
print('X in dataset:', dataset[0].shape)
print('Y in dataset:', dataset[1].shape)
...
def define_generator(latent_dim, n_classes=18):
    ...
    gen = Dropout(0.8, name='Generator_Dense_Layer_Dropout')(gen)
    ...
    gen = Conv1DTranspose(filters=54, kernel_size=3, padding='same', strides=2,  name='Generator-Hidden-Layer-1')(merge)  # 16x107x64
    ...
    gen = Conv1DTranspose(filters=108, kernel_size=3, padding='same', strides=2, name='Generator-Hidden-Layer-2')(gen)  # 16x107x64
...
def generate_real_samples(dataset, n_samples):
    ...
    y = noisy_labels(y, 0.05)  # Flipping the labels with 5% Probability, to fool the Discriminator.
    return [X, labels], y
...
def generate_fake_samples(generator, latent_dim, n_samples):
    ...
    y = noisy_labels(y, 0.05)  # flip labels with 5% probability
...
def saveResultsCSV(gen_activation, epochs, batch_per_epoch, d_loss_real, d_loss_fake, g_loss):
    ...
    fileString = './results/CGAN_CONV1D_LSTM_250epochs_32Batch_withMajority_Label_smoothing&noise.csv'
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch):
    ...
    g_model.save('CGAN_CONV1D_LSTM_250epochs_32Batch_withmajority_Labelsmoothing_noisy.h5')
