...
def define_discriminator(in_shape=(32, 107, 1), n_classes=17):
    ...
    dis = Conv2D(filters=64, kernel_size=(3, 3), strides=(2, 2), padding='same', kernel_constraint=const,
                 name='Discriminator_Hidden_Layer_1')(merge)  # 16x54x64
...
def define_generator(latent_dim, n_classes=17):
    ...
    gen = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 1), padding='same',
                          name='Generator_Hidden_Layer_1')(merge)  # 8x107x128
    ...
    gen = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 1), padding='same',
                          name='Generator_Hidden_Layer_2')(gen)  # 16x107x128
    ...
    gen = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 1), padding='same',
                          name='Generator_Hidden_Layer_3')(gen)  # 32x107x64
    # gen = BatchNormalization(momentum=0.5)(gen)
...
def saveResultsCSV(gen_activation, n_steps, d_loss_real, d_loss_fake, g_loss):
    path = './results/'
    if not os.path.exists(path):
        os.makedirs(path)

    fileString = './results/WCGAN_10_epochs_32Batch_Minority_withoutBatchNormalization_labelSmoothing_conv2d_lstm.csv'
    file = Path(fileString)
    now = datetime.now()

    if not file.exists():
        f = open(fileString, 'w')
        f.write('gen_activation; n_steps; d_loss_real; d_loss_fake; g_loss\n')
        f.close()
    with open(fileString, "a") as f:
        f.write('{};{};{};{};{}\n'.format(gen_activation, n_steps, d_loss_real, d_loss_fake, g_loss))
    f.close()
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch, n_critic=5):
    ...
    for i in range(n_steps):
        ...
        for _ in range(n_critic):
            ...
            d1_tmp.append(d_loss_real)
            # generate 'fake' examples
            [X_fake, labels], y_fake = generate_fake_samples(g_model, latent_dim, half_batch)
            # update discriminator model weights
            d_loss_fake, _ = d_model.train_on_batch([X_fake, labels], y_fake)
            d2_tmp.append(d_loss_fake)
            ...
            print('>%d, d1=%.3f, d2=%.3f g=%.3f' % (i + 1, d1_hist[-1], d2_hist[-1], g_loss))
            plot_history(d1_hist, d2_hist, g_hist)
            saveResultsCSV(gen_activation, i, d_loss_real, d_loss_fake, g_loss)
    # save the generator model
    g_model.save('WCGAN_10_epochs_32Batch_Minority_withoutBatchNormalization_labelSmoothing_conv2d_lstm.h5')
