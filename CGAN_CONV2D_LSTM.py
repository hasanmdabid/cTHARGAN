...
trainx_min = trainx_min.reshape(nr_samples * nr_rows, nr_columns)  # To sclae the data converting from 3D to 2D

max_val_Unscaled = np.max(trainx_min)
min_val_Unscaled = np.min(trainx_min)
print('Maximume_value Before scaled:', max_val_Unscaled)
print('Minimume_value Before scaled:', min_val_Unscaled)
...
X = trainx_min_scaled.reshape(nr_samples, nr_rows, nr_columns, 1)  # Reshaping from 2D to 4D
...
def define_discriminator(in_shape=(32, 107, 1), n_classes=17):
    ...
    merge = Concatenate(name='Discriminator_Combine_Layer')(
    ...
    dis = Conv2D(filters=64, kernel_size=(3, 3), strides=(2, 2), padding='same', name='Discriminator_Hidden_Layer_1')(
        merge)  # 16x54x64
    dis = LeakyReLU(alpha=0.2, name='Discriminator_Hidden_Layer_Activation_1')(dis)
    dis = MaxPooling2D(padding='same')(dis)
    dis = Dropout(0.5)(dis)
    # down-sample
    dis = Conv2D(filters=128, kernel_size=(3, 3), strides=(2, 2), padding='same', name='Discriminator_Hidden_Layer_2')(
        dis)  # 8x27x128
    ...
    """
    # Dense Layer
    dis = Dense(512, name='Discriminator-Label-Hidden-Dense-Layer1')(dis)
    dis = Dense(256, name='Discriminator-Label-Hidden-Dense-Layer2')(dis)
    dis = Dense(128, name='Discriminator-Label-Hidden-Dense-Layer3')(dis)
    dis = Dense(64, name='Discriminator-Label-Hidden-Dense-Layer4')(dis)
    dis = Dense(16, name='Discriminator-Label-Hidden-Dense-Layer5')(dis)

    """
    # output
    out_layer = Dense(1, activation='sigmoid', name='Discriminator_Output_Layer')(dis)  # Shape=1
...
def define_generator(latent_dim, n_classes=17):
    ...
    merge = Concatenate(name='Generator-Combine-Layer')(
        [gen, li])  # Shape=4x107x64 (Extra channel corresponds to the label)

    # up-sample to 16x16 ====== HIDDEN layer 1
    gen = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 1), padding='same', name='Generator_Hidden_Layer_1')(merge)  # 8x107x128
    ...
    gen = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 1), padding='same', name='Generator_Hidden_Layer_2')(gen)  # 16x107x128
    ...
    gen = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 1), padding='same', name='Generator_Hidden_Layer_3')(gen)  # 32x107x64
    # gen = BatchNormalization(momentum=0.5)(gen)
...
def saveResultsCSV(gen_activation, epochs, batch_per_epoch, d_loss_real, d_loss_fake, g_loss):
    ...
    fileString = './results/CGAN_500_epochs_32Batch_Minority_withoutBatchNormalization_labelSmoothing_conv2d_lstm.csv'
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs, n_batch):
    ...
    for i in range(n_epochs):
        ...
        for j in range(bat_per_epo):
            ...
            #if (i % 1000) == 0:
            #    g_model.save(f'CGAN_{i}_epochs_32Batch_Minority_withoutBatchNormalization_labelSmoothing_noisy_conv2d_lstm.h5')

    # save the generator model
    g_model.save('CGAN_500_epochs_32Batch_Minority_withoutBatchNormalization_labelSmoothing_conv2d_lstm.h5')
